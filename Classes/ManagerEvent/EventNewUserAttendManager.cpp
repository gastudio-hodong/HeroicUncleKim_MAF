//
//  EventNewUserAttendManager.cpp
//  FantasyClicker-mobile
//
//  Created by MAF_TG on 2022/08/03.
//
#include "EventNewUserAttendManager.h"
...
EventNewUserAttendManager::EventNewUserAttendManager(void):
_nResult(0),
_nEndTime(0),
...
bool EventNewUserAttendManager::init()
{
    return true;
}

#pragma mark - get set
const int EventNewUserAttendManager::getToday() const
{
    int result = 0;
...
const int EventNewUserAttendManager::getEndTime() const
{
    return _nEndTime;
}
const bool EventNewUserAttendManager::isRewardCondition() const
{
    return _isRewardCondition;
}
const bool EventNewUserAttendManager::isNewUserEventCondition() const
{
    bool result = false;
    
    if(EventManager::getInstance()->isActivatedEvent(35) == true && _nResult == 1)
        result = true;
    
    return result;
}
const Vector<InfoEventNewAttendReward*> EventNewUserAttendManager::getListInfoReward() const
{
    return _listInfoReward;
}

const InfoEventNewAttendReward* EventNewUserAttendManager::getEventNewAttendReward(const int day) const
{
    InfoEventNewAttendReward* result = nullptr;
...
#pragma mark - network
void EventNewUserAttendManager::requestInfo(const std::function<void(bool, int)>& callback)
{
    _callbackInfo = callback;
    //
    std::string url = "/event/newuserattendance/info/%d";
...
void EventNewUserAttendManager::responseInfo(cocos2d::network::HttpResponse* const response,const std::string &data)
{
    bool bError = false;
    ...
    for ( int i = 0; i < jsonParser["_reward_list"].Size(); i++ )
    {
        ...
        auto objInfoReward = InfoEventNewAttendReward::create();
...
void EventNewUserAttendManager::requestReward(const std::function<void(bool, int)>& callback)
{
    _callbackReward = callback;
    
    //
    std::string url = "/event/newuserattendance/reward";
...
void EventNewUserAttendManager::responseReward(cocos2d::network::HttpResponse* const response,const std::string &data)
{
    bool bError = false;
    if ( response == nullptr || response->isSucceed() == false || response->getResponseCode() != 200 )
    {
