...
PopupDice* PopupDice::create(int nDice)
{
    PopupDice* pRet = new(std::nothrow) PopupDice();
...
#pragma mark - init
void PopupDice::initVar()
{
    
}

void PopupDice::initUI()
{
    ...
    if ( _nDice <= 0 )
    {
        strTitle = GAME_TEXT("t_ui_event_boardgame_outpnt_1");
    }
...
void PopupDice::uiError()
{
    ...
    _spriteContainer->addChild(layerButton);
    {
        ...
        // item
        auto itemClose = MafNode::MafMenuItemSprite::create(Sprite::create("Assets/ui_common/popup_btn_no.png"), nullptr, CC_CALLBACK_1(PopupDice::onClickClose, this));
        menu->addChild(itemClose);
        {
            auto labelText = Label::createWithTTF(GAME_TEXT("t_ui_event_boardgame_outpnt_bt"), GAME_FONT, 24);
...
//        layerContents->addChild(spriteBG);
        ...
        auto labelText = Label::createWithTTF(GAME_TEXT("t_ui_event_boardgame_outpnt_2"), GAME_FONT, 25);
        labelText->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
        labelText->setPosition(layerContents->getContentSize()/ 2);
...
void PopupDice::setCallbackClose(std::function<void(void)> callback)
{
    _callbackClose = callback;
}

#pragma mark - click
void PopupDice::onClickClose(cocos2d::Ref* sender)
{
    if ( _callbackClose != nullptr )
    {
        _callbackClose();
