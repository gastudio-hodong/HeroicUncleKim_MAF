//
//  PopupEventSunfishLevelResult.cpp
//  FantasyClicker-mobile
//
//  Created by MAF_Mac on 09/01/2020.
//

#include "PopupEventSunfishLevelResult.h"
...
PopupEventSunfishLevelResult::PopupEventSunfishLevelResult(void) :
_bInitUI(false),
...
bool PopupEventSunfishLevelResult::init()
{
    if ( !PopupBase::init() )
    {
        return false;
...
void PopupEventSunfishLevelResult::onEnter()
{
    PopupBase::onEnter();
...
#pragma mark - init
void PopupEventSunfishLevelResult::initVar()
{
    
}

void PopupEventSunfishLevelResult::initUi()
{
    // size
    _spriteContainer->setContentSize( Size(700, 818) );
    
    // label title
    _uiTitle = ui::CText::create(GAME_TEXT("t_ui_event_sunfish_popup_title_2"), GAME_FONT, 32);
    ...
    uiClose->addClickEventListener(CC_CALLBACK_1(PopupEventSunfishLevelResult::onClickClose, this));
    _spriteContainer->addChild(uiClose);
}

#pragma mark - ui
void PopupEventSunfishLevelResult::uiContainer()
{
...
void PopupEventSunfishLevelResult::uiInfo()
{
...
void PopupEventSunfishLevelResult::uiButtons()
{
    ...
    layout->addChild(uiButtons);
    {
        ...
        uiOk->addClickEventListener(CC_CALLBACK_1(PopupEventSunfishLevelResult::onClickClose, this));
...
#pragma mark - ui : draw
void PopupEventSunfishLevelResult::drawInfo()
{
    auto layout = _uiContentsInfo;
    ...
    if ( auto uiInfoTitle = utils::findChild<ui::Layout*>(layout, "UI_INFO_TITLE"); uiInfoTitle != nullptr )
    {
        ...
        if ( _bResult == true )
        {
            auto uiIconLobster = ui::ImageView::create(objSunfishLevelNext->getSkinPath());
        ...
        {
            auto uiIconLobster = ui::ImageView::create(objSunfishLevelPrev->getSkinPath());
            uiIconLobster->setPosition(Vec2(uiInfoTitle->getContentSize().width / 2, 99));
            ((ui::Scale9Sprite*)uiIconLobster->getVirtualRenderer())->getTexture()->setTexParameters(texParams);
            uiIconLobster->setScale(MIN((uiInfoTitle->getContentSize().height * 0.6) / uiIconLobster->getContentSize().width, (uiInfoTitle->getContentSize().height * 0.6) / uiIconLobster->getContentSize().height));
    ...
    if ( auto uiInfoDesc = utils::findChild<ui::Layout*>(layout, "UI_INFO_DESC"); uiInfoDesc != nullptr )
    {
        std::string desc = "";
        if ( _bResult == true )
            desc = objSunfishLevelNext->getSurvivalText();
        else
            desc = objSunfishLevelNext->getDieText();
        desc = MafUtils::format(desc.c_str(), objSunfishLevelNext->getPoint());
...
void PopupEventSunfishLevelResult::drawButtons()
{
    auto layout = _uiContentsButton;
...
void PopupEventSunfishLevelResult::onClickClose(Ref* sender)
{
    //
    if ( _bResult == true )
    {
        EventSunfishManager::getInstance()->onDelegateGameLevelUp();
    }
    else
    {
...
#pragma mark - set, get
void PopupEventSunfishLevelResult::setLevelIdx(int idx)
{
    _levelIdx = idx;
...
void PopupEventSunfishLevelResult::setResult(bool bResult)
{
    _bResult = bResult;
