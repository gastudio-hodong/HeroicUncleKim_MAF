//
//  PopupEventReturnAttend.cpp
//  FantasyClicker-mobile
//
//  Created by MAF_TG on 2022/06/13.
//

#include "PopupEventReturnAttend.h"
...
PopupEventReturnAttend* PopupEventReturnAttend::create()
{
    PopupEventReturnAttend* pRet = new(std::nothrow) PopupEventReturnAttend();
    if ( pRet && pRet->init() )
    {
        pRet->autorelease();
        return pRet;
...
PopupEventReturnAttend::PopupEventReturnAttend(void):
...
#pragma mark -init
bool PopupEventReturnAttend::init()
{
    if ( !PopupBase::init())
    {
        return false;
    ...
    _listInfoReward = EventReturnAttendManager::getInstance()->getListInfoReward();
    _nEndTime = EventReturnAttendManager::getInstance()->getEndTime();
    _isRewardCondition = EventReturnAttendManager::getInstance()->isRewardCondition();
    
    initVar();
    initUI();
    
    StoryManager::getInstance()->showStory((int)E_COMIC::EVENT_RETURN_ATTEND);
    return true;
}

void PopupEventReturnAttend::onEnter()
{
    PopupBase::onEnter();
    
    PopupLoading::show();
    EventReturnAttendManager::getInstance()->requestInfo(CC_CALLBACK_2(PopupEventReturnAttend::callbackInfo, this));
}

void PopupEventReturnAttend::initVar()
{
...
void PopupEventReturnAttend::initUI()
{
    //650
...
#pragma mark -ui
void PopupEventReturnAttend::uiRefresh()
{
    uiTopBanner();
    uiPopupTable();
    uiPopupBottom();
...
void PopupEventReturnAttend::uiTopBanner()
{
    _lyTopBanner->removeAllChildren();
    
    auto sprBg = Sprite::create("Assets/ui/return_event/return_top_bg.png");
    sprBg->getTexture()->setTexParameters(texParams);
    sprBg->setScale(3);
    sprBg->setPosition(_lyTopBanner->getContentSize()/2);
    _lyTopBanner->addChild(sprBg);
    
    auto lbTitle = Label::createWithTTF(GAME_TEXT("t_ui_return_event_title"), GAME_FONT, 32);
    ...
    auto lbInfo = Label::createWithTTF(GAME_TEXT("t_ui_return_event_msg_1"), GAME_FONT, 24);
...
void PopupEventReturnAttend::uiPopupTable()
{
    ...
    {
        auto objInfoReward = EventReturnAttendManager::getInstance()->getEventReturnAttendReward(i+1);
...
void PopupEventReturnAttend::uiPopupBottom()
{
    ...
    btnClose->addClickEventListener(CC_CALLBACK_1(PopupEventReturnAttend::onClickClose, this));
...
#pragma mark -click
void PopupEventReturnAttend::onClickClose(Ref* const sender)
{
    hide();
}

#pragma mark -callback
void PopupEventReturnAttend::callbackInfo(const bool bResult, const int nResult)
{
    PopupLoading::hide();
    ...
    _nTodayReward = EventReturnAttendManager::getInstance()->getToday();
    _listInfoReward = EventReturnAttendManager::getInstance()->getListInfoReward();
    _nEndTime = EventReturnAttendManager::getInstance()->getEndTime();
    _isRewardCondition = EventReturnAttendManager::getInstance()->isRewardCondition();
    ...
    {
        PopupLoading::show();
        EventReturnAttendManager::getInstance()->requestReward(CC_CALLBACK_2(PopupEventReturnAttend::callbackReward, this));
    }
    else
        uiRefresh();
}

void PopupEventReturnAttend::callbackReward(const bool bResult, const int nResult)
{
    PopupLoading::hide();
